...
namespace CookiesAuth.Reveal
{
    internal class SampleUserContextProvider : IRVUserContextProvider
    {
        public IRVUserContext GetUserContext(HttpContext aspnetContext)
        {
            ...
            // RVUserContext is a default implementation of IRVUserContext, which allows to store properties in addition to the userId,
            // these properties can be used later for example in the authentication provider.
            // You could store data related to the current request this way. In this case, we're just storing a sample header data.
